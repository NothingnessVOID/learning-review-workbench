...
Mac 双击 **更新并启动工作台.command**，检查源码和锁文件后重新构建并安全重启本项目实例。平时仍用 **启动学习工作台.command**；若运行中的是旧构建，启动器会明确提示更新。只安装 package-lock.json 锁定的依赖，不自动升级版本。设置页显示实际运行版本与构建指纹。

日常可以从“继续学习”回到主题，随时“记一句”。旧记录、搜索与授权／导出选择器支持翻页；选中记录后可预览“交给我的 Agent”的内容，再复制或下载。外部复盘独立归档，不覆盖原话。

完整备份可选包含运行上下文。更新记录和本轮实测范围见 [改进跟踪](docs/IMPROVEMENT_TRACKER.md)；真实资料包和本人日常路径的效果仍由实际使用验证。
